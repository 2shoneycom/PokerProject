...
public class PokerHandEvaluator
{
    ...
    public Tuple<int, float> EvaluateHand()
    {
        ...
        {
            nums[i] = PokerGameControl.Card.GetCardNum(idxs[i]);
            shapes[i] = PokerGameControl.Card.GetCardShape(idxs[i]);
        }
    ...
    private float IsStraightFlush()
    {
        ...
        if (is_flush && is_straight)
        {
            ...
            scr += 0.1 * ShapesToInt(shapes[0]);    // 플러쉬 계열은 한 장의 문양만 보면 됨
        }
    ...
    private float IsStraight()
    {
        ...
        scr += 0.1 * ShapesToInt(shapes[0]);    // 제일 높은 한 장의 문양만 보면 됨 (아마?)
        return scr;
    }
    ...
    private float IsTwoPair()
    {
        ...
        if (nums[0] == nums[1] && nums[2] == nums[3])
        {
            ...
            scr += 0.1 * math.Max(ShapesToInt(shapes[0]), ShapesToInt(shapes[1]));  // 높은 쌍(7,7)에서 높은 문양을 점수로 사용
        }
        ...
        else if (cardlen >= 5 && nums[1] == nums[2] && nums[3] == nums[4])
        {
            ...
            scr += 0.1 * math.Max(ShapesToInt(shapes[1]), ShapesToInt(shapes[2]));  // 높은 쌍(4,4)에서 높은 문양을 점수로 사용
        }
    ...
    private float IsOnePair()
    {
        ...
        if (nums[0] == nums[1])
        {
            ...
            scr += 0.1 * math.Max(ShapesToInt(shapes[0]), ShapesToInt(shapes[1]));  // 페어에서 높은 문양을 점수로 사용
        }
        else if (cardlen >= 3 && nums[1] == nums[2])
        {
            // ex) 7,5,5,3,1
            scr = 1000000 * nums[1] + 10000 * nums[0] + 100 * nums[3] + nums[4];
            scr += 0.1 * math.Max(ShapesToInt(shapes[1]), ShapesToInt(shapes[2]));  // 페어에서 높은 문양을 점수로 사용
        }
        else if (cardlen >= 4 && nums[2] == nums[3])
        {
            // ex) 7,5,3,3,1
            scr = 1000000 * nums[2] + 10000 * nums[0] + 100 * nums[1] + nums[4];
            scr += 0.1 * math.Max(ShapesToInt(shapes[2]), ShapesToInt(shapes[3]));  // 페어에서 높은 문양을 점수로 사용
        }
        else if (cardlen >= 5 && nums[3] == nums[4])
        {
            // ex) 13,10,9,5,5
            scr = 1000000 * nums[3] + 10000 * nums[0] + 100 * nums[1] + nums[2];
            scr += 0.1 * math.Max(ShapesToInt(shapes[3]), ShapesToInt(shapes[4]));  // 페어에서 높은 문양을 점수로 사용
        }
